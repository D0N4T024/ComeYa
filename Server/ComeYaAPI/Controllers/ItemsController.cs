...
namespace ComeYaAPI.Controllers
{
    ...
    public class ItemsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ReadItemDTO>>> GetAllItems([FromQuery]List<string>? types,[FromQuery] List<string>? categories,decimal price= 0M, int page=0, ulong combo=2, int restaurant=0, int rand =0, decimal pageSize = 0M, int marketinImg=0 )
        {
           
            var items = await _unitOfWork.Items.GetAllItems(types,categories,price,page,combo,restaurant,rand,pageSize, marketinImg);
